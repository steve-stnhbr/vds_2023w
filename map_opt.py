...
def create_map_graph(fig, highlight_locations=[], level=3, year=2022, selected_data=[], color_urban_types=True, color_population_density=True):
    ...
    # Check if the year is available in the data
    ...
    # Handle highlighting and selection logic
    highlight_locations = get_geos_with_urban_types(highlight_locations) if len(highlight_locations) > 0 else []
    if selected_data:
        highlight_locations = intersection(highlight_locations, selected_data)

    if fig is None:
        # Initialize the figure and layout only once
        fig = create_figure()
        fig.update_layout(
            go.Layout(
        ...
        # Loop through urban types and add traces
        for urban_type in URBAN_TYPES.values():
            df_population_density_use = df_population_density[~df_population_density['geo'].isin(highlight_locations + selected_data)]
            df_population_density_use = df_population_density[df_population_density['urban_type'] == urban_type]
            add_choropleth_trace(fig, df_population_density_use, current_geojson, urban_type, color_urban_types, color_population_density, year)

    else:
        # Update existing traces
        update_existing_traces(fig, df_population_density, highlight_locations, selected_data, color_urban_types, color_population_density, year, max_value)

    # Update map layout if selected data is available
    if selected_data:
...
# Helper function to add a choropleth trace
def add_choropleth_trace(fig, df, geojson, urban_type, color_urban_types, color_population_density, year):
    locations = df['geo']
    values = df[str(year)]
    fig.add_trace(
        go.Choroplethmapbox(
            ...
            text=pd.DataFrame(df['geo']).apply(lambda x: get_geo_name(x['geo']) or x['geo'], axis=1),
...
# Helper function to update existing traces
def update_existing_traces(fig, df_population_density, highlight_locations, selected_data, color_urban_types, color_population_density, year, max_value):
    opacity = UNHIGHLIGHT_OPACITY if highlight_locations or selected_data else BASE_OPACITY

    for i, urban_type in enumerate(URBAN_TYPES.values()):
        data = fig['data'][i]
        chunk = df_population_density[~df_population_density['geo'].isin(highlight_locations + selected_data)]
        ...
        for urban_type in get_urban_types_of_geos(selected_data, as_string=True, unique=True):
            ...
            fig['data'].append(
                go.Choroplethmapbox(
                    ...
                    featureidkey=GEO_IDENTIFIER,
                    z=values if color_population_density else [NO_DENSITY_VALUE] * len(locations),
                    marker={'opacity': SELECTED_OPACITY if not highlight_locations else SELECTED_UNHIGHLIGHT_OPACITY, "line": {"width": .3}},
                    colorscale=LOG_COLORSCALES[urban_type] if color_urban_types else LOG_COLORSCALE,
    ...
    if highlight_locations:
        for urban_type in get_urban_types_of_geos(highlight_locations, as_string=True, unique=True):
            values = df_population_density[df_population_density['geo'].isin(highlight_locations)][str(year)] / max_value
            fig['data'].append(
                go.Choroplethmapbox(
                    geojson=current_geojson,
