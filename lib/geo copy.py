...
df_geo_names = pd.read_csv(ROOT_DIR + '/geo/nuts3-names.csv', 
                           sep=';',
                           header=0, 
                           names=['country', 'nuts3', 'name', 'bla1', 'bla2', 'bla3', 'bla4', 'bla5', 'bla6'], 
                           usecols=['country', 'nuts3', 'name'])
df_geo_names = pd.merge(df_geo_names, pd.read_csv(ROOT_DIR + '/geo/country_codes.csv', usecols=['country_name', 'alpha-2']), left_on='country', right_on='alpha-2', how='left').drop(['alpha-2'], axis=1)
